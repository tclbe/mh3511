...
(j): Based on the summations, we can clearly see poker had a net gain, while roulette had a net loss.
Clearly, don't play roulette.
...
Note that matrix multiplication is done using `%*%` in R.
The `solve()` function attempts to find the inverse of a square matrix.
To verify it is the inverse, we can perform matrix multiplication.
